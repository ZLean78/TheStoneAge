...
onready var ai = get_node("nav/EnemyAI")
onready var all_enemy_units=ai.all_enemy_units
...
func _process(_delta):
	...
	for i in range(0,all_enemy_units.size()):
		all_enemy_units[i]._animate()
		if !all_enemy_units[i].sprite.is_playing():
			all_enemy_units[i].sprite.play(all_enemy_units[i].sprite.animation)
...
func get_entities_in_area(area):
	var e=[]
	for entity in all_entities:
		if entity.position>area[0].x and entity.position.x<area[1].x:
			if entity.position.y>area[0].y and entity.position.y<area[1].y:
				e.append(entity)
	return e
...
func area_selected(obj):
	...
	var et = get_entities_in_area(area)
