...
func _check_units_health():
	for unit in units:		
		if its_raining && !unit.is_sheltered && !unit.is_cave_sheltered && counter_timer.timeout:
			unit.health -= 5	
		if unit.health <= 0:
			unit.queue_free()
			units.erase(unit)
			
		if unit.is_sheltered || unit.is_cave_sheltered && unit.health <100:
			unit.health += 3
		
		if unit.health > 100:
			unit.health = 100
		unit.progress_bar.value = unit.health
